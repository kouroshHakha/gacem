...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument('--use-model', dest='use_model', default=False, action='store_true',
                        help='If True loads the model from root_dir in spec_file')
...
if __name__ == '__main__':
    ...
    if _args.init_pop_path:
        alg = alg_cls(_args.spec_fpath, load=_args.load, init_buffer_path=_args.init_pop_path,
                      use_model=_args.use_model)
    else:
        alg = alg_cls(_args.spec_fpath, load=_args.load, use_model=_args.use_model)
