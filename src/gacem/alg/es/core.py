...
class SimpleESPolicy(abc.ABC):

    def __init__(self, num_params, sigma_init, popsize, weight_decay):
...
class CMAES(SimpleESPolicy):
    '''CMA-ES wrapper.'''
    def __init__(self, num_params,      # number of model parameters
                 sigma_init=0.10,       # initial standard deviation
                 popsize=255,           # population size
                 weight_decay=0.01):    # weight decay coefficient
        super().__init__(num_params, sigma_init, popsize, weight_decay)
        self.solutions = None
...
class Actor:
    ...
    VALID_PTYPES = ['CMAES', 'SimpleGA', 'OpenES', 'PEPG']
    def __init__(self, observation_space, action_space, **policy_kwargs):
        super().__init__()

        # only support Box action space for now
        assert isinstance(action_space, Box)
        # only support single dimensional action spaces for now
        assert len(action_space.shape) == 1

        try:
            ptype = policy_kwargs.pop('policy_type')
            if ptype not in self.VALID_PTYPES:
                raise ValueError(f'policy_type is not supported. valid options are {self.VALID_PTYPES}')
        except KeyError:
            raise ValueError('policy_type is not specified in actor parameters.')

        pi_class = eval(ptype)
        self.pi: SimpleESPolicy = cast(SimpleESPolicy, pi_class(**policy_kwargs))

    def rms_stdev(self):
        return self.pi.rms_stdev()

    def ask(self):
        return self.pi.ask()

    def tell(self, fitnesses):
        return self.pi.tell(fitnesses)
