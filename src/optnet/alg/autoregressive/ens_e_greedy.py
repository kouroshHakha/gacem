...
from utils.pytorch import get_l1_reg
...
class AutoRegSearch(AlgBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.snep = params['snep']
        self.eps_decay_rate = params['eps_decay_rate']
        self.l1_coef = params['l1_coef']
        self.eps = 1
        ...
        self.model_unvisited: Optional[Ensemble] = None
        self.buffer = None
        self.opt = None
        ...
        self.setup_model(params['models'], params['models_u'])

    def setup_model(self,
                    model_params: List[Dict[str, Any]],
                    unvisited_params: List[Dict[str, Any]]):
        dim = self.ndim
        module_list = []
        ...
        umodule_list = []
        for i, model in enumerate(unvisited_params):
            umodule_list.append(MADE(dim, model['hiddens'], dim * 3 * model['nr_mix'],
                                    seed=self.seed + 10 * i, natural_ordering=True))
        params = self.specs['params']
    ...
    def run_epoch(self, data: np.ndarray, weights: np.ndarray, mode='train', model_type='original'):
        if model_type == 'original':
            model = self.model
            opt = self.opt
        else:
            model = self.model_unvisited
            opt = self.opt_unvisited
        ...
        for step in range(nstep):
            ...
            xin = xb_tens[:, 0, :]
            if model_type == 'original':
                loss = model.get_nll(xin, wb_tens)
            else:
                loss = model.get_nll(xin, ll_type='logpp') + self.l1_coef * get_l1_reg(model)
            if mode == 'train':
                opt.zero_grad()
                loss.backward()
        ...
        while n_collected < n_samples:
            if uniform:
                ...
                if random.random() >= self.eps:
                    ...
                    _, xnew_ind = self.model_unvisited.sample_model(1, self.bsize,
    ...
    def train(self, iter_cnt: int, nepochs: int, split=1.0):
        ...
        for epoch_id in range(nepochs):
            ...
            for _ in range(self.snep):
                tr_unvisited_nll = self.run_epoch(xtr, wtr, mode='train', model_type='unvisited')
                tr_unvisited_profit += tr_unvisited_nll / self.nepochs
                print(f'[unvisited_{iter_cnt}] epoch {epoch_id} loss = {tr_unvisited_nll}')
    ...
    def save_checkpoint(self, iter_cnt, tr_losses, avg_cost):
        dict_to_save = dict(
            ...
            model_u_state=self.model_unvisited.state_dict(),
            opt_u_state=self.opt_unvisited.state_dict(),
        )
    ...
    def load_checkpoint(self, ckpt_path: Union[str, Path]):
        ...
        self.model_unvisited.load_state_dict(checkpoint['model_u_state'])
        # override optimizers with input parameters
        self.opt = optim.Adam(self.model.parameters(), self.lr)
        self.opt_unvisited = optim.Adam(self.model_unvisited.parameters(), self.lr)
    ...
    def _run_alg(self):
        ...
        while iter_cnt < self.niter:
            ...
            if (iter_cnt + 1) % 10 == 0 and self.ndim == 2:
                self._save_2d_samples(self.model_unvisited, iter_cnt, 1000, name='unvisited_states')
                self._save_2d_samples(self.model, iter_cnt, 1000, name='dist')
